<?php
class ModuleImage extends WP_Widget {

	public function __construct() {		
		$id_base = 'module-image';
		$name	= 'ModuleImage';
		$widget_options = array(
					'classname' => 'module-image',
					'description' => 'ModuleImage'
				);
	...
	public function form( $instance ) {	
		...
		$inputID 	= $this->get_field_id('img_url');
		$inputName 	= $this->get_field_name('img_url');
		$inputValue = @$instance['img_url'];
		$class = array("widefat");
		$html		= $vHtml->label('ImgUrl',array('for'=>$inputID))
					. $vHtml->cmsTextbox($inputID,$inputName,"widefat",$inputValue);
		echo $vHtml->pTag($html);	

		$inputID 	= $this->get_field_id('img_link');
		$inputName 	= $this->get_field_name('img_link');
		$inputValue = @$instance['img_link'];
		$class = array("widefat");
		$html		= $vHtml->label('ImgLink',array('for'=>$inputID))
					. $vHtml->cmsTextbox($inputID,$inputName,"widefat",$inputValue);
		echo $vHtml->pTag($html);	
